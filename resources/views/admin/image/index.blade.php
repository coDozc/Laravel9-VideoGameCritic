...
<h2>{{$game->title}}</h2>
    <form role="form" action="{{route('admin.image.store', ['gid'=>$game->id])}}" method="post" enctype="multipart/form-data">
        @csrf
            <label>Title</label>
            ...
            <table class="table">
                ...
                @foreach( $images as $rs )
                    <tr>
                        ...
                        <td><a href="{{route('admin.image.destroy',['gid'=>$game -> id, 'id'=>$rs -> id])}}"class="btn btn-danger">Delete</a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
